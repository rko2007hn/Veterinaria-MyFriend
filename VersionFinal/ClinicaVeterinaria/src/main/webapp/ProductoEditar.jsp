...
<div id="apDiv1"></div>
<div id="apDiv2">
  <form id="form1" name="form1" method="post" action="ProductoModificarServlet">
  
<%@page import="clinicaveterinaria.modelo.Producto;"%>
<%
Producto vo = (Producto)request.getAttribute("vo");
%>
  
    <table width="100%" border="1">
      <tr bgcolor="#D1F09D">
        <td colspan="6" bgcolor="#D1F09D"><div align="center"><strong>PRODUCTO</strong><strong> : MODIFICAR REGISTRO</strong></div></td>
      </tr>
      <tr>
      
        <td width="15%" bgcolor="#D1F09D"><span class="Estilo5">ID</span></td>
        <td width="2%" bgcolor="#D1F09D"><span class="Estilo5">:</span></td>
        <td width="27%" bgcolor="#D6E9DE"><label>
          <input type="text" name="id" id="id" value="<%=vo.getIdProducto()%>" readonly="readonly" />
        </label></td>
        ...
        <td width="2%" bgcolor="#D1F09D"><span class="Estilo5">:</span></td>
        <td width="27%" bgcolor="#D6E9DE"><label>
          <input type="text" name="nombre" id="nombre" value="<%=vo.getProducto()%>"/>
        </label></td>
        ...
        <td width="25%" bgcolor="#D6E9DE"><label>
          <input type="text" name="precio" id="precio" value="<%=vo.getPrecio()%>" />
        </label></td>
      </tr>
      <tr>
		...
		<select name="cbotipo" id="cbotipo" value="<%=vo.getoTipoProducto().getIdTipo()%>">
	          <%
				for(TipoProducto c:lista){
					//
					
					if (vo.getoTipoProducto().getIdTipo()==c.getIdTipo()) {
						out.println("<option value="+c.getIdTipo()+" selected>"+c.getTipo()+"</option>");
					}else{	
